...
class FomentoController extends My_Controller {
	public function __construct(){
		parent::__construct();
		$this->load->model('FomentoModel');
	...
	public function listFomento(){
		...
		$count = $this->FomentoModel->FomentoModelo($filtros);
		...
		$sidx = (isset($_POST['sidx']) && $_POST['sidx'] != "") ? $_POST['sidx'] : 'pk_fomento';
		$sord = (isset($_POST['sidx']) && $_POST['sidx'] != "") ? $_POST['sord'] : 'ASC';
		$filtros['sidx'] = $sidx;
		$filtros['sord'] = $sord;
		$dataControl = $this->FomentoModel->FomentoModelo($filtros);
		if($count > 0){
		...
		foreach($dataControl as $row){
			$response->rows[$i]['id'] = $row['pk_fomento'];
			$response->rows[$i]['cell'] = array('pk_fomento' => $row['pk_fomento'],
				"btnEditar" => '<button type="button" class="btn btn-labeled btn-primary btn-sm btnEditar" data-bs-toggle="modal" data-bs-target="#modalInventario" data-id="'.$row['pk_fomento'].'"><span class="btn-label"><i class="bi bi-pencil"></i></span>  Editar</button>',
				"nombre" => $row['nombre'],
	...
	public function guardarFomento(){
		if(!$this->session->userdata('pb_idUsuario')){
		...
		$res = $this->FomentoModel->fomentoGuardar($_POST);
		echo json_encode($res);die();
	}
	public function loadFormulario(){
		$data['pk_fomento'] = $this->input->post('pk_fomento');
		$data['INFO']['nombre'] = '';
		$data['INFO']['fk_asociacion'] = '';
		$data['INFO']['fk_disciplina'] = '';
	
		if(intval($data['pk_fomento']) > 0){//Editar
			$data['INFO'] = $this->FomentoModel->obtenerRegistroPorId($data['pk_fomento']);
		}
